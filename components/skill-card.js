...
const SkillCard = ({skill,percentage}) => {
  return (
    ...
    <Image src={skillIconClassNames[skill]|| otherIcons[skill] || `https://cdn.jsdelivr.net/gh/devicons/devicon/icons/${skill}/${skill}-original.svg`} alt={skill} layout="fill"/>
    </div>
    <div>
    <h6 className='text-md'>{skill.toUpperCase()}</h6>
        ...
        <div className='bg-first w-4/5 h-2 rounded-full flex'>
            {/* percentage */}
            <div className='bg-green-200 h-2 rounded-full' style={{ width:`${percentage}%` }}></div>
        </div>
